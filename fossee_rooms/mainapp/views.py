...
def hotelhome(request):
    return render(request, 'homefiles/hotelhome.html')
...
def dashboard(request):
    ...
    user_bookings = Booking.objects.filter(
        customer=request.user).order_by('-booked_on')

    # All Bookings
    all_bookings = Booking.objects.all().order_by(
        '-date', 'room', 'time_slot__start_time', 'cancelled')
    ...
    if rooms.exists():
        ...
        if rooms.start_date > today_date:
            rooms_old = Rooms.objects.filter(
                end_date=rooms.start_date-timedelta(days=1))
            if rooms_old.exists():
                rooms_old = rooms_old[0]
                rooms_context1 += str(rooms_old.rooms) + \
                    ' (Upto ' + str(rooms_old.end_date) + ')'
                rooms_context2 += str(rooms.rooms) + \
                    ' (From ' + str(rooms.start_date) + ')'
        else:
    ...
    for time_slot in time_slots:
        ...
        if time_slot.end_date is None:
            ...
            if time_slot.start_date > today_date:
                time_slot_dict['remark'] = '(From ' + \
                    str(time_slot.start_date) + ')'
            else:
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('submit') == 'book-room':
            ...
            booking_obj = Booking(customer=request.user, date=booking_date,
                                  room=booking_room, time_slot=booking_time_slot, room_manager=room_manager)
            booking_obj.save()
        ...
        # Update Pre-Booking Allowance
            ...
            # Show Success Message on dashboard
            messages.success(
                request, 'Pre-Booking Allowance changed successfully!')
        ...
        # Update Number of Rooms
            ...
            if bookings.exists() and bookings[0].date > today_date:
                ...
                rooms_obj = Rooms(
                    rooms=new_rooms, start_date=new_rooms_start_date)
                rooms_obj.save()
        ...
        # Add new Time Slots
            ...
            # Other overlapped time slots (need not to be deleted)
            ...
            for time_slot in time_slots:
                ...
                if time_slot.end_date is None:
                    if time_overlaps(new_start_time, new_end_time, time_slot.start_time, time_slot.end_time):
                        overlapped_time_slot_dict['start_time'] = str(
                            time_slot.start_time)
                        overlapped_time_slot_dict['end_time'] = str(
                            time_slot.end_time)
                        if time_slot.start_date > today_date:
                            overlapped_time_slot_dict['remark'] = '(From ' + str(
                                time_slot.start_date) + ')'
                        else:
                            overlapped_time_slot_dict['remark'] = ''
                        del_time_slots_list.append(
                            overlapped_time_slot_dict.copy())
            ...
            # If no time slot needs to be deleted but some already deleted time slots are overlapping
                ...
                new_time_slot = TimeSlot(
                    start_time=new_start_time, end_time=new_end_time, start_date=new_start_date)
                new_time_slot.save()
            ...
            # If no time slot overlaps
                ...
                # Create new time slot instance
                new_time_slot = TimeSlot(
                    start_time=new_start_time, end_time=new_end_time, start_date=today_date)
                new_time_slot.save()
                ...
                if time_slot.end_date is None:
                    if time_overlaps(new_start_time, new_end_time, time_slot.start_time, time_slot.end_time):
                        ...
                        booking = Booking.objects.filter(
                            cancelled=False, time_slot=time_slot).order_by('-date')
...
def setprofile(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            # Updating Profile Model
            profile_obj = Profile(
                user=user_obj, gender=gender, contact_no=contact_no, address=address)
            profile_obj.save()
...
def bookingDateChangedAJAX(request):
    ...
    if allowance.exists() and days_diff > allowance[0].days:
        data['error'] = "*You can only book for " + \
            str(allowance[0].days) + " days in advance!"
        return JsonResponse(data)
        ...
        if booking_date > today_date or (booking_date == today_date and time_slot.start_time >= time_now):
            ...
            if time_slot.start_date <= booking_date and (time_slot.end_date is None or time_slot.end_date >= booking_date):
                if not Booking.objects.filter(cancelled=False, date=booking_date, room=booking_room, time_slot=time_slot).exists():
                    time_slot_dict['id'] = time_slot.id
                    time_slot_dict['slot'] = str(
                        time_slot.start_time) + ' - ' + str(time_slot.end_time)
